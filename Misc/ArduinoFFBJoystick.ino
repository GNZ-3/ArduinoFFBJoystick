...
int16_t mymap(int16_t myval,int16_t minval,int16_t maxval)
{
  if(myval > maxval){
    return ENCODER_MAX_VALUE; 
  }else if(myval < minval){
    return ENCODER_MIN_VALUE;
  }
  return map(myval,minval,maxval,ENCODER_MIN_VALUE,ENCODER_MAX_VALUE);
}
...
void loop() {
  ...
  Serial.print(valuex);
  int procx = mymap(valuex,valueminx,valuemaxx);
  Joystick.setXAxis(procx);

  Serial.print("\tEncY:");
  valuey = analogRead(A1);
  Serial.print(valuey);
  int procy = mymap(valuey,valueminy,valuemaxy);
  Joystick.setYAxis(procy);

  Serial.print("\tEncZ:");
  valuez = analogRead(A2);
  Serial.print(valuez);
  int procz = mymap(valuez,valueminz,valuemaxz);
  Joystick.setZAxis(procz);

// Fix no spring effect in game
/*
...
*/
  ...
  effectparams[0].springPosition = procx;
  effectparams[1].springMaxPosition = ENCODER_MAX_VALUE;
  effectparams[1].springPosition = procy;
